...
import { Package, AlertCircle, CheckCircle, TrendingDown, BarChart3, Archive } from "lucide-react"
...
export function Dashboard() {
  ...
  const {products, loading, error, success}= useSelector((state: RootState) =>state.inventory)
...
const handleaddProduct=(payload: CreateProduct)=>{
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white">Inventory Tracker</h1>
          </div>
          <p className="flex text-white">Real-time warehouse inventory management system</p>
        </div>

        {error && (
          <div
            className="mb-4 border rounded-lg p-4 flex items-start gap-3"
          >
        ...
        {!loading && products.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="bg-card border border-border rounded-lg p-4 border-neutral-950">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-xs font-mono text-orange-200 uppercase tracking-wide mb-1">Total Products</p>
                  <p className="text-3xl font-bold text-orange-600">{totalProducts}</p>
                </div>
                <Package className="w-6 h-6 text-orange-600" />
              </div>
              <p className="text-xs text-orange-200">Active items in warehouse</p>
            </div>

            <div className="bg-card border border-border rounded-lg p-4 border-neutral-950">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-xs font-mono text-yellow-100 uppercase tracking-wide mb-1">Total Stock</p>
                  <p className="text-3xl font-bold text-primary text-yellow-400">{totalStock}</p>
                </div>
                <BarChart3 className="w-6 h-6 text-accent/60 text-yellow-400" />
              </div>
              <p className="text-xs text-yellow-100">Units across all products</p>
            </div>

            <div className="bg-card border border-border rounded-lg p-4 border-amber-500/30 bg-amber-500/5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-xs font-mono text-amber-600 uppercase tracking-wide mb-1">Low Stock</p>
            ...
            <div className="bg-card border border-border rounded-lg p-4 border-red-600 bg-red-950/30">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-xs font-mono text-red-400 uppercase tracking-wide mb-1">Out of Stock</p>
                  <p className="text-3xl font-bold text-red-500">{criticalStockProducts.length}</p>
                </div>
          ...
          <div className="mb-8 bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-amber-600 mb-2">Low Stock Alerts</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {lowStockProducts.map((product) => (
                    ...
                    >
                      <div>
                        <p className="font-mono text-xs text-amber-900">{product.sku}</p>
                        <p className="text-amber-900 truncate">{product.name}</p>
                      </div>
        ...
        {criticalStockProducts.length > 0 && (
          <div className="mb-8 bg-red-950/30 border border-red-600 rounded-lg p-4">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-red-500 mb-2">Out of Stock</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {criticalStockProducts.map((product) => (
                    ...
                    >
                      <div>
                        <p className="font-mono text-xs text-red-900">{product.sku}</p>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="bg-primary hover:bg-primary/90 bg-amber-500 text-primary-foreground rounded-lg px-6 py-3 font-semibold transition-colors whitespace-nowrap"
          >
            + Add Product
          </button>
        </div>
        ...
        {!loading && products.length === 0 && (
          <div className="border border-dashed border-border rounded-lg p-12 text-center">
            ...
            <p className="text-muted-foreground mb-6">Get started by adding your first product to the inventory.</p>
            <button
              onClick={() => setformOpen(true)}
        ...
        {!loading && filteredProducts.length > 0 && (
          <div>
            <div className="mb-6">
              <h2 className="flex text-lg font-bold text-foreground mb-1">Inventory List</h2>
      ...
      <AddProduct isOpen={isformOpen} onClose={() => setformOpen(false)} onSubmit={handleaddProduct} />
    </div>
  )
}
